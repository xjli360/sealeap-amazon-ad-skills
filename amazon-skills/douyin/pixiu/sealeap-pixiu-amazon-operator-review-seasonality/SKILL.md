---
name: sealeap-pixiu-amazon-operator-review-seasonality
description: "聚焦季节性、库存管理、发货与入库的 Amazon 跨境经营复盘。在需要将经验判断转成可验证的问题，识别适用前提、替代解释和可迁移的经营动作时使用；根据当前业务问题选择证据卡，避免加载无关主题。"
---

# Amazon 跨境经营复盘：季节性、库存管理、发货与入库

## 目标

围绕季节性、库存管理、发货与入库，将经验判断转成可验证的问题，识别适用前提、替代解释和可迁移的经营动作。
...
- **预算分配**：按活动角色、边际回报、库存和利润约束分配预算，并设置停止条件。
...
完整的 32 张主题证据卡见 [references/topic-cards.md](references/topic-cards.md)；37 种原有组合见 [references/scenario-patterns.md](references/scenario-patterns.md)。
