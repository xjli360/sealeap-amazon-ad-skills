---
name: sealeap-pixiu-amazon-fba-inventory-fees
description: "聚焦平台费用、季节性、库存管理的 Amazon FBA、物流与库存。在需要核对可售、在途、交期和费用，量化断货与积压风险，形成可执行的库存处理方案时使用；根据当前业务问题选择证据卡，避免加载无关主题。"
---

# Amazon FBA、物流与库存：平台费用、季节性、库存管理

## 目标

围绕平台费用、季节性、库存管理，核对可售、在途、交期和费用，量化断货与积压风险，形成可执行的库存处理方案。
...
完整的 24 张主题证据卡见 [references/topic-cards.md](references/topic-cards.md)；19 种原有组合见 [references/scenario-patterns.md](references/scenario-patterns.md)。
