---
name: sealeap-baize-amazon-ad-optimization-order
description: "Optimize Amazon ads in an attribution-safe order: placement allocation first, irrelevant-query controls second, and target-level bid changes last. Use for 先调广告位还是先否词、竞价越调越乱、商品页流量差、搜索词清理. Do not apply negatives mechanically."
---
...
仅在自有数据不足且当前任务确实需要外部证据时，读取 [references/mcp-data-plan.md](references/mcp-data-plan.md)，再使用 `scripts/mcp_research.py`。本 Skill 的外部取数目的：补充关键词相关性和自然可见度代理证据，最终以广告搜索词报告为准。
